...
public class AsyncEnumerableTests {
    ...
    public async Task FromEventPattern_ConcurrentEventFirings_NeverThrow() {
        for (var i = 0; i < 1000; i++) {
            ...
            var enumerator = StorageAsyncEnumerable
    ...
    public async Task FromEventPattern_RemovesHandlerOnCancellation() {
        ...
        using var cts = new CancellationTokenSource();
        var enumerator = StorageAsyncEnumerable.FromEventPattern<Action<object, int>, int>(Add, Remove, cts.Token).GetAsyncEnumerator();
